...
std::vector<at::Tensor> top_pool_backward(
    ...
    auto max_val = at::zeros({batch, channel, width}, at::kFloat);
    auto max_ind = at::zeros({batch, channel, width}, at::kLong);
    ...
    auto gt_mask    = at::zeros({batch, channel, width}, at::kByte);
    auto max_temp   = at::zeros({batch, channel, width}, at::kFloat);
    for (int32_t ind = 1; ind < height; ++ind) {
        input_temp = input.select(2, height - ind - 1);
        // at::gt_out(gt_mask, input_temp, max_val); // bug
        gt_mask = at::gt(input_temp, max_val);
